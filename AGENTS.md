...
`Packages/EverywhereCore/` contains the gomobile bridge and its local Swift package; generated XCFrameworks stay untracked. `ThirdParty/zashboard/` is a checked-in vendor bundle. Xcode and SwiftPM wiring is maintained in `Everywhere.xcodeproj/` and `Scripts/wire_project.rb`.
...
There is no XCTest target. Run `go test ./...` in `Packages/EverywhereCore/go` for bridge changes and verify its module graph contains neither sing-box nor xray-core. Before submitting, run the simulator build and manually check the affected UI flow. Test a real tunnel on device when modifying `EverywhereNE/`, entitlements, or VPN settings.
